# -*- coding: utf-8 -*-
"""
Created on Fri Jan 15 11:06:30 2016
...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 16 15:12:18 2015
...
import concurrent.futures, xlsxwriter, requests, textwrap, os, time, tweepy,threading
home = expanduser("~")
LOCK = threading.Lock()
...
def collect_reshares(dirname,driver,hashtag):
   ...
   # driver.get('https://www.facebook.com/shares/view?id='+str(post_id))
    ...
    reshares=[]
    firts_part= source.split('</h5><div class=""><div class="_5pcp"><span><span class="fsm fwn fcg"><a class="_5pcq" href="')
...
def collect_reshare_results(url,dirname):
  ...
  #  driver.get("https://www.facebook.com")
    ...
    try:
        ...
        img.crop((195,133,img.size[0]-300,img.size[1])).save('%s/Desktop/%s/Facebook_screenshots/%s.png' %(home,dirname,post_id)) 
    except:
        pass
    source=str(driver.page_source)
  ...
  #  with suppress(Exception):
    ...
    driver.quit()
    try:
        authortw=source.split('class="_5pb8 _29h _303" href="https://www.facebook.com/')[1].split('?')[0]
    except:
        authortw=author
...
def count_followers(url,author_id):
    ...
    audience=0  
    if 'permalink' in url:
        facebook_profile='href="https://www.facebook.com/profile.php?id=%s&amp;sk=followers"' %(author_id)       
    ...
    source=driver.page_source
    if 'photo' in url and not 'photos' in url:
        facebook_profile=source.split('"page_uri":"')[1].split('\"')[0].replace('\\','')+'/'+'followers'
    try:
...
#    driver = webdriver.Chrome('/Users/War7/Desktop/chromedriver')
    ...
    #audience=0
    if 'permalink' in url:
        facebook_profile='href="https://www.facebook.com/profile.php?id=%s&amp;sk=friends"' %(author_id)       
    if 'permalink' not in url and 'photos' not in url:           
...
#    driver.save_screenshot('/Users/War7/Desktop/%s.png' %(author_id))
    if 'photo' in url and not 'photos' in url:
        facebook_profile=source.split('"page_uri":"')[1].split('\"')[0].replace('\\','')+'/'+'friends'
   # try:
    mightbeaudience=source.split(facebook_profile)
    #except:
     #   return 0    
    for el in mightbeaudience:
        if '</a></span></div></div></div></div></div><div' in el:
            try:
                audience=int(el.split('</a></span></div></div></div></div></div><div')[0].lstrip('>').replace(',',''))   
            except:
                pass
    driver.quit()
    return  audience
...
def collect_people_who_reshare_results(author_id,url,followers_data,dirname,fromdate,todate,el):
    ...
    try:
        link = "https://graph.facebook.com/v2.5/%s?fields=likes,name&access_token=%s" %(author_id,logg_details['token'])
...
def collecting_reshares_everything(tags,dirname,followers_data,fromdate,todate):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:        
        result=list(executor.map(partial(process_tag_post,dirname=dirname,followers_data=followers_data,fromdate=fromdate,todate=todate), reshares))      
...
def process_tag_post(el,followers_data,dirname,fromdate,todate): 
        author,authortw,author_id,post_id,likes,comments,shares,createddate,createdtime =(collect_reshare_results(el,dirname))
        audience=(collect_people_who_reshare_results(author_id,el,followers_data,dirname,fromdate,todate,el))[1]
        ...
        if author not in followers_data['Facebook']:
            ...
            if audience==0:
                try:
                    audience=floor(followers_data['Facebook'][author]/postscount_data['Facebook'][author])
                except:
                    audience=0
                followers_data['Facebook'][author]+=audience
            else:
