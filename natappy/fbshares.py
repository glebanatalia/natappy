...
def collect_reshares(post_id,dirname,driver):
    ...
    if len(reshares)>0:
        ...
        img.crop((374,53,img.size[0]-486,img.size[1])).save('%s/Desktop/%s/Facebook_shares/%s.png' %(home,dirname,post_id))
    return reshares
...
def count_page_likes(url,driver):
    ...
    if 'permalink' in url:
        ...
        facebook_profile='href="https://www.facebook.com/profile.php?id=%s&amp;sk=likes"' %(author_id)     
    ...
    for el in mightbeaudience:
        if ' people like this</div>' in el:
            try:
                audience=int(el.split(' people like this</div>')[0].split('">')[2].replace(',',''))
            except:
                pass
...
def collecting_reshares_everything(post_id,dirname):
    ...
    for el in reshares:
        ...
        reshares_collection[post_id]={'link':el,'likes':likes,'comments':comments,'shares':shares, 'author':resharer, 'impression':int(audience)}
            ...
            for blogger in reshares_collection[media].keys():
                for post in reshares_collection[media][blogger].keys(): 
                    ...
                    for share in reshares_collection[media][blogger][post].keys():  
                        link1='www.facebook.com/%s'%(post) if media=='Facebook' else 'www.twitter.com/statuses/%s'%(post)
                        share_details=[blogger,media,str(post),link1,reshares_collection[media][blogger][post][share]['author'],reshares_collection[media][blogger][post][share]['link'],reshares_collection[media][blogger][post][share]['likes'],reshares_collection[media][blogger][post][share]['comments'],reshares_collection[media][blogger][post][share]['shares'],reshares_collection[media][blogger][post][share]['impression'] ]
...
def excel(nice_format,dirname):  
    ...
    fieldsnames= ('BLOGGER','MEDIA','POST ID','ORIGINAL LINK','AUTHOR','SHARED LINK','LIKES','COMMENTS','SHARES','IMPRESSION')
    col = 0
