...
info = {'blogpost': {}, 'comptags': [], 'competition': [], 'campaign': '', 'brandname': '', 'excludetags': [], 'bloggers': [],
        'tags': [], 'format': None, 'platforms': [], 'fromdate': [], 'todate': [], 'replies': False, 'retweets': False, 'blogposts': []}

passwords = [line.split('=')[1].split('\'')[1] for line in open(
    '%s/Desktop/natappy/passwords/passwords.rtf' % home, 'r') if '=' in line]
...
def first_window(root, info=info):
    ...
    def next_():
        info['platforms'] = [chkbox.get() for chkbox in [variablefb, variableig,
                                                         variabletw, variablebl] if chkbox.get() != 'no']
        info['retweets'] = True if retweets.get() == 'True' else False
        info['replies'] = True if replies.get() == 'True' else False
        info['competition'] = [chkbox.get() for chkbox in [compvariablefb, compvariableig,
                                                           compvariabletw] if chkbox.get() != 'no']
    ...
    def question(info=info):
        ...
        if variabletw.get() == 'Twitter':
            additional_button3 = ttk.Checkbutton(
                c, text='Include retweets', variable=retweets, onvalue='True', offvalue='False')
            additional_button3.grid(column=0, row=5, sticky=(N, W, E), padx=50, pady=5)
            add_buttons.append(additional_button3)
            additional_button4 = ttk.Checkbutton(
                c, text='Include replies', variable=replies, onvalue='True', offvalue='False')
    ...
    g0 = ttk.Checkbutton(c, text='Blogs', variable=variablebl,
                         onvalue='Blogs', offvalue='no', command=active_next)
    g0.grid(column=0, row=1, sticky=(N, W, E, S), padx=20, pady=(20, 10))

    g1 = ttk.Checkbutton(c, text='Facebook', variable=variablefb,
                         onvalue='Facebook', offvalue='no', command=active_next)
    g1.grid(column=0, row=2, sticky=(N, W, E, S), padx=20, pady=10)

    g2 = ttk.Checkbutton(c, text='Instagram', variable=variableig,
                         onvalue='Instagram', offvalue='no', command=active_next)
    g2.grid(column=0, row=3, sticky=(N, W, E, S), padx=20, pady=10)

    g3 = ttk.Checkbutton(c, text='Twitter', variable=variabletw,
                         onvalue='Twitter', offvalue='no', command=question)
...
#    compg1.grid(column=1, row=2, sticky=(N,W,E,S),padx=20, pady=10)
    ...
    compg2 = ttk.Checkbutton(c, text='collect competition entries',
                             variable=compvariableig, onvalue='Instagram', offvalue='no', state='disabled')
    compg2.grid(column=1, row=3, sticky=(N, W, E, S), padx=20, pady=10)

    compg3 = ttk.Checkbutton(c, text='collect competition entries',
                             variable=compvariabletw, onvalue='Twitter', offvalue='no')
...
def add_blogger(info=info):
    ...
    tip1 = ListboxToolTip(screen_name, textwrap.wrap(
        'Name that will be shown in report. It might be whatever, but it can not contains commas.  ', 25))

    link_name = ttk.Label(c, text="Facebook link name")
    link_name.grid(column=1, row=1, padx=(0, 0), pady=20)
    tip2 = ListboxToolTip(link_name, textwrap.wrap(
        "Name shown in facebook link ex. https://www.facebook.com/blogger_link_name. If link name is in format link_name-somenumbers then please, provide numbers instead of name", 23))
    ...
    tip3 = ListboxToolTip(instagram_name, textwrap.wrap(
        "Name shown in instagram link Ex. https://www.instagram.com/lydiaemillen/ link_name=lydiaemillen", 25))
    ...
    tip4 = ListboxToolTip(twitter_name, [
                          "Copy Paste name", "shown in twitter link Ex.", "twitter.com/TimeOutLondon",  "link_name=TimeOutLondon"])
    ...
    connections = {t1: [ct1, bt1, dt1, dtw1, a1], t2: [ct2, bt2, dt2, dtw2, a2], t3: [
        ct3, bt3, dt3, dtw3, a3], t4: [ct4, bt4, dt4, dtw4, a4], t5: [ct5, bt5, dt5, dtw5, a5]}
...
def third_window(root, info):
    store0, store1, store2, store3, store4, store5 = StringVar(
    ...
    def next_window():
        ...
        for entry2 in entries2:
            if entry2.get() and entry2.get() not in info['excludetags']:
                info['excludetags'].append(str(entry2.get()).encode(
...
def fifth_window(root, info):
    ...
    def back(info=info):
        info['fromdate'] = [int(fromyear.get()), (month_to_number(
            frommonth.get())), int(fromday.get())]
    ...
    def info_completed(info=info):
        ...
        try:
            ...
            if not info['competition']:
                nat.create_report(info['bloggers'], info['platforms'], info['tags'], info['format'], info['fromdate'], info['todate'], info[
                                  'replies'], info['retweets'], info['excludetags'], info['brandname'], info['campaign'], info['blogpost'])
                root.withdraw()
                messagebox.showinfo("Report completed", 'Details: \n Report: {format} \n Media: {platform2} \n Tags: {tags2} \n Bloggers: {bloggers2} \n Report completed'.format(tags2=' '.join(
                    [tag for tag in info['tags']]), platform2=' '.join([platform for platform in info['platforms']]), bloggers2=' '.join([blogger for blogger in info['bloggers']]), **info), parent=c)
            else:
                campcomp.campaign_competition(info['blogpost'], info['comptags'], info['competition'], info['campaign'], info['brandname'], info['excludetags'], info[
    ...
    frommonth = Spinbox(c, values=['January', 'February', 'March', 'April', 'May', 'June', 'July',
                                   'August', 'September', 'October', 'November', 'December'], textvariable=frommonthvar, width=11)
    ...
    tomonth = Spinbox(c, values=['January', 'February', 'March', 'April', 'May', 'June', 'July',
                                 'August', 'September', 'October', 'November', 'December'], width=11, textvariable=tomonthvar)
...
def bloggers():
    bloggers = sorted([blogger for blogger in pandas.read_excel(
        '%s/Desktop/natappy/bloggers.xlsx' % home)['Blogger'] if type(blogger) is not float])
    return bloggers
