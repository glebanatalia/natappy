# -*- coding: utf-8 -*-
"""
Created on Fri Feb  5 16:33:22 2016
...
def campaign_competition(blogpost,comptags,competition,campaign,brandname,excludetags,bloggers,tags,repformat,platforms,fromdate,todate,replies,retweets,blogposts): 
    ...
    for platform in [plat for plat in competition if plat not in['Facebook','Blogs']]:   
        with suppress(Exception):
            os.makedirs('%s/Desktop/%s/%s_screenshots'%(home,dirname,platform)) 
        all_media[platform]=compet.competition(platform,bloggers,followers_data,startdate=fromdate,searchQuery=comptags,maxTweets=maxTweets,tweetsPerQry=tweetsPerQry,max_id=max_id,sinceId=sinceId)
    ...
    pickle.dump(followers_data, open('{home}/Desktop/natappy/pickle/followers_data.pickle'.format(home=home), 'wb'))
    
    followers_data= pickle.load(open('{home}/Desktop/natappy/pickle/followers_data.pickle'.format(home=home), 'rb'))  
...
#---------------------------------------------------------------------------------------------------------#
    ...
    for platform in [plat for plat in platforms if plat not in ['Blogs']]: 
        all_media2[platform]=natappy.get_post_with_tags(bloggers,platform,tags,repformat,fromdate,todate,followers_data2,replies,retweets,excludetags,dirname) 
    
    
    pickle.dump(all_media2, open('{home}/Desktop/natappy/pickle/all_media2.pickle'.format(home=home), 'wb'))
    pickle.dump(followers_data2, open('{home}/Desktop/followers2.pickle'.format(home=home), 'wb'))
    
    all_media2= pickle.load(open('{home}/Desktop/natappy/pickle/all_media2.pickle'.format(home=home), 'rb'))
    followers_data2=pickle.load(open('{home}/Desktop/natappy/pickle/followers2.pickle'.format(home=home), 'rb'))
                    ...
                    if post_info:
                        if blogger in blogposts:
                            if blogposts[blogger]:
                                newpdf.blogger_blogposts_page(rep,dirname,blogger,blogposts,post_info)
...
if __name__ == '__main__':
    info= {'replies': False, 'comptags': ['peonyliminzanziba'], 'fromdate': [2016, 2, 9], 'blogposts': [], 'todate': [2016, 2, 16], 'blogpost': {'Peony Lim': ['http://peonylim.com/']}, 'bloggers': ['Peony Lim'], 'retweets': False, 'format': 'Full', 'campaign': '', 'brandname': 'competition+bloggers+blog', 'tags': ['peonyliminzanziba'], 'platforms': ['Facebook', 'Instagram', 'Twitter', 'Blogs'], 'excludetags': ['Exclude tag'], 'competition': ['Instagram', 'Twitter']}

    campaign_competition(info['blogpost'],info['comptags'],info['competition'],info['campaign'],info['brandname'],info['excludetags'],info['bloggers'],info['tags'],info['format'],info['platforms'],info['fromdate'],info['todate'],info['replies'],info['retweets'],info['blogposts'])
