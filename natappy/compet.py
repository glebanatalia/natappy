...
import concurrent.futures,requests,os, time, tweepy,threading
...
def process_post(post,platform,logg_details,bloggers,followers_data,startdate):
    ...
    if platform=='Instagram':
        ...
        onepost= process_post_instagram(logg_details,post,author_name,bloggers,followers_data,startdate)   
    return onepost

def get_followers_count_ig(user_id,logg_details):
    link='https://api.instagram.com/v1/users/%s?access_token=%s' %(user_id,logg_details['token'])   
...
def process_post_instagram(logg_details,post,author_name,bloggers,followers_data,fromdate):
    ...
    time_=time_=datetime(int(str(post.created_time)[:10].split('-')[0]),int(str(post.created_time)[:10].split('-')[1]),int(str(post.created_time)[:10].split('-')[2]))                                     
    if post.caption and post.caption.text and time_>=from_ :
        if 'smashbox' in str(post.caption.text).lower() or 'bootsuk' in str(post.caption.text).lower():
            ...
            take_screenshot(str(post.link),str(post.user.username)+'_'+str(post.id), note='regular')
...
def collect_posts_insta(autho,logg_details,bloggers,followers_data,fromdate,searchQuery):
    ...
    while next_ and counter<70:            
        more_media, next_ = autho.tag_recent_media(count=100,tag_name=searchQuery,with_next_url=next_)
...
def process_post_twitter(tweet,author_name,bloggers,followers_data,startdate):  
    ...
    likes=tweet.retweeted_status.favorite_count if hasattr(tweet, 'retweeted_status') else tweet.favorite_count 
    retweet=tweet.retweeted_status.retweet_count if hasattr(tweet, 'retweeted_status') else tweet.retweet_count 
    replied=True if tweet.in_reply_to_screen_name!=None and tweet.in_reply_to_status_id_str!=None else False
    time_=time_=datetime(int(str(tweet.created_at)[:10].split('-')[0]),int(str(tweet.created_at)[:10].split('-')[1]),int(str(tweet.created_at)[:10].split('-')[2])) 
    ...
    while tweetCount < maxTweets and Run:
        try:
            ...
            for tweet in new_tweets:
                ...
                tweet_time=datetime(int(str(tweet.created_at)[:10].split('-')[0]),int(str(tweet.created_at)[:10].split('-')[1]),int(str(tweet.created_at)[:10].split('-')[2]))         
                if tweet_time<from_:
                    Run=False  
                else:
...
def analyse_competition(logg_details,platform,bloggers,followers_data,autho,query,startdate,maxTweets,tweetsPerQry,max_id,sinceId):   
    if platform=='Twitter':     
        posts=collect_posts_twitter(autho,bloggers,followers_data,startdate=startdate,searchQuery=query,maxTweets=maxTweets,tweetsPerQry=tweetsPerQry,max_id=max_id,sinceId=sinceId)
    if platform=='Instagram':
        posts=collect_posts_insta(autho,logg_details,bloggers,followers_data,startdate,searchQuery=query)
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        result = list(executor.map(partial(process_post,logg_details=logg_details,platform=platform,bloggers=bloggers,followers_data=followers_data,startdate=startdate),posts))  
...
def competition(platform,bloggers,followers_data,startdate,searchQuery,maxTweets,tweetsPerQry,max_id,sinceId):
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        result = list(executor.map(partial(process_query,bloggers=bloggers,followers_data=followers_data,startdate=startdate,maxTweets=maxTweets, tweetsPerQry=tweetsPerQry, max_id=max_id, sinceId=sinceId,platform=platform),searchQuery))
...
def process_query(query,bloggers,startdate,maxTweets,tweetsPerQry,max_id,sinceId,platform,followers_data):
    ...
    query_results= analyse_competition(logg_details,platform,bloggers,followers_data,autho,query,startdate=startdate,maxTweets=maxTweets,tweetsPerQry=tweetsPerQry,max_id=max_id,sinceId=sinceId)            
    ...
    try:
        ...
        if platform=='Twitter':       
            ...
            try:
                ...
                browser.quit()
                cut_screenshot('%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,platform,name),url=url,note=note,platform=platform,dirname=dirname,name=name)
...
def collect_screenshots(dirname):
    ...
    print('Screenshots collected. Elapsed:', time.time()-tic, 'Average:',int( time.time()-tic)/len(links),' per image.' )
...
def competreport(platforms,tags,fromdate,bloggers,todate,repformat,brandname,campaign):
    ...
    for platform in [plat for plat in platforms if plat not in['Facebook']]:       
        all_media[platform]=competition(platform,bloggers,followers_data,startdate=fromdate,searchQuery=tags,maxTweets=maxTweets,tweetsPerQry=tweetsPerQry,max_id=max_id,sinceId=sinceId)
    if 'Facebook' in platforms:
        with suppress(Exception):
            os.makedirs('%s/Desktop/%s/Facebook_screenshots'%(home,dirname)) 
        all_media['Facebook']=fbcompet.collecting_reshares_everything(tags,dirname=dirname,followers_data=followers_data,fromdate=fromdate,todate=todate)
    ...
    pickle.dump(followers_data, open('{home}/Desktop/followers_data.pickle'.format(home=home), 'wb'))
    
    followers_data= pickle.load(open('{home}/Desktop/followers_data.pickle'.format(home=home), 'rb'))  
    ...
    compdf.pdf(all_media,dirname,bloggers,tags,fromdate,platforms,followers_data,brandname='SmashboxUK',campaign='BeLegendary')
    
if __name__ == '__main__':
    info= {'todate': [2016, 5, 11], 'blogposts': [], 'excludetags': ['Exclude tag'], 'blogpost': {}, 'replies': False, 'competition': ['Instagram'], 'format': 'Full', 'fromdate': [2016, 4, 10], 'tags': ['belegendary'], 'comptags': ['belegendary'], 'retweets': False, 'brandname': 'BeLegendary', 'campaign': 'SmashboxUK', 'bloggers': [], 'platforms': ['Instagram']}
  #  info= {'campaign': 'Fabulous in Dubai', 'format': 'Full', 'retweets': False, 'excludetags': [], 'fromdate': [2016, 1, 29], 'kind': 'competition', 'tags': ['fabulousindubai'], 'todate': [2016, 2, 5], 'replies': False, 'brandname': 'My Report',  'platforms': ['Instagram', 'Twitter'], 'bloggers': [], 'brand': {'Instagram': None, 'Twitter': None, 'Facebook': None, 'Name': None}}
    ...
    competreport(platforms=info['platforms'],tags=info['tags'],fromdate=info['fromdate'],bloggers=info['bloggers'],todate=info['todate'],repformat=info['format'],brandname=info['brandname'],campaign=info['campaign'])    
