# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 13:44:49 2016
        ...
        if screenname_to_linkname(blogger,media) in all_media[media].keys():
            for post in all_media[media][screenname_to_linkname(blogger,media)].keys():
                ...
                with suppress(Exception):
                    ...
                    if ratio>1:                   
                        position='landscape' 
                    screenshots_paths[post_path]={'width':img.size[0],'height':img.size[1],'position':position}
...
def choose_screenshots_layout(screenshots_paths):
    ...
    for el in screenshots_paths.keys():
        layout[el]={'counter':counter,'width':screenshots_paths[el]['width'],'position':screenshots_paths[el]['position']}
        counter+=1
    final_layout={}
...
def blogger_rates_count(media,reachtype,blogger,all_media):
    ...
    if reachtype=='Applause Rate':
       with suppress(Exception):
            rate=blogger_likes_count(media,blogger,all_media)/len(all_media[media][screenname_to_linkname(blogger,media)])
            return    str(("%.2f" %rate))
    if reachtype=='Conversation Rate':
        with suppress(Exception):
            rate=blogger_comments_count(media,blogger,all_media)/len(all_media[media][screenname_to_linkname(blogger,media)])
            return    str(("%.2f" %rate))
    if reachtype=='Amplification Rate':
        with suppress(Exception):
            rate= blogger_shares_count(media,blogger,all_media)/len(all_media[media][screenname_to_linkname(blogger,media)])
            return    str(("%.2f" %rate))
    if reachtype=='Interaction Rate':
        with suppress(Exception):
            rate= blogger_interactions_count(media,blogger,all_media)/len(all_media[media][screenname_to_linkname(blogger,media)])
            return    str(("%.2f" %rate))
    return toreturn
    ...
    if all_media['Facebook']:
        ...
        for blogger in bloggers:
            try:
                ...
                #try:
                    rep.drawString(135,position,str("{:,}".format(len(all_media['Facebook'][screenname_to_linkname(blogger,'Facebook')]))))
                    rep.drawString(245,position,str("{:,}".format(blogger_likes_count('Facebook',blogger,all_media))))
                    rep.drawString(300,position,str("{:,}".format(blogger_comments_count('Facebook',blogger,all_media))))
                    rep.drawString(365,position,str("{:,}".format(blogger_shares_count('Facebook',blogger,all_media))))
                    rep.drawString(185,position,str("{:,}".format(blogger_interactions_count('Facebook',blogger,all_media))))
                    rep.drawString(415,position,str(blogger_rates_count('Facebook','Interaction Rate',blogger,all_media)))
                    rep.drawString(490,position,str(blogger_rates_count('Facebook','Applause Rate',blogger,all_media)))
                    rep.drawString(565,position,str(blogger_rates_count('Facebook','Conversation Rate',blogger,all_media)))
                    rep.drawString(650,position,str(blogger_rates_count('Facebook','Amplification Rate',blogger,all_media)))
                    rep.drawString(715,position,str("{:,}".format(blogger_reach_count('Facebook','Direct',blogger,all_media, followers_data))))
                    rep.drawString(763,position,str("{:,}".format(blogger_reach_count('Facebook','Impressions',blogger,all_media,followers_data))))
                    counter+=30  
                else:
                    rows-=1
            except:
        ...
        for blogger in bloggers:
            try:
                ...
                if len(all_media['Instagram'][screenname_to_linkname(blogger,'Instagram')])!=0:
                    position=450-counter
                    rep.drawString(25,position,blogger)
                    rep.drawString(150,position,str("{:,}".format(len(all_media['Instagram'][screenname_to_linkname(blogger,'Instagram')]))))
                    rep.drawString(270,position,str("{:,}".format(blogger_likes_count('Instagram',blogger,all_media))))
                    rep.drawString(335,position,str("{:,}".format(blogger_comments_count('Instagram',blogger,all_media))))
                    rep.drawString(190,position,str("{:,}".format(blogger_interactions_count('Instagram',blogger,all_media))))
                    rep.drawString(490,position,str(blogger_rates_count('Instagram','Applause Rate',blogger,all_media)))
                    rep.drawString(590,position,str(blogger_rates_count('Instagram','Conversation Rate',blogger,all_media)))
                    rep.drawString(410,position,str(blogger_rates_count('Instagram','Interaction Rate',blogger,all_media)))
                    rep.drawString(670,position,str("{:,}".format(blogger_reach_count('Instagram','Direct',blogger,all_media, followers_data))))
                    rep.drawString(736,position,str("{:,}".format(blogger_reach_count('Instagram','Impressions',blogger,all_media,followers_data))))
                    counter+=30 
                else:
                    rows-=1
            except:
        ...
        for blogger in bloggers:
            try:
                ...
                if len(all_media['Twitter'][screenname_to_linkname(blogger,'Twitter')])!=0:
                    position=450-counter
                    rep.drawString(25,position,blogger)                
                    rep.drawString(150,position,str("{:,}".format(len(all_media['Twitter'][screenname_to_linkname(blogger,'Twitter')]))))
                    rep.drawString(278,position,str("{:,}".format(blogger_likes_count('Twitter',blogger,all_media))))
                    rep.drawString(332,position,str("{:,}".format(blogger_shares_count('Twitter',blogger,all_media))))
                    rep.drawString(203,position,str("{:,}".format(blogger_interactions_count('Twitter',blogger,all_media))))
                    rep.drawString(502,position,str(blogger_rates_count('Twitter','Applause Rate',blogger,all_media)))
                    rep.drawString(590,position,str(blogger_rates_count('Twitter','Amplification Rate',blogger,all_media)))
                    rep.drawString(410,position,str(blogger_rates_count('Twitter','Interaction Rate',blogger,all_media)))
                    rep.drawString(675,position,str("{:,}".format(blogger_reach_count('Twitter','Direct',blogger,all_media, followers_data))))
                    rep.drawString(740,position,str("{:,}".format(blogger_reach_count('Twitter','Impressions',blogger,all_media, followers_data))))
                    counter+=30  
                else:
                    rows-=1
            except:
...
def welcome_pages(rep,brand,campaign):
    rep.drawImage('%s/Desktop/natappy/images/first.png' %home,200,280,470,213)  #260,320,300,100
    rep.setFont('Lato-Lig', 24)
    ...
    rep.drawString(160,300,'WaR is a diverse digital network consisting of bespoke bloggers and online magazines. WaR presents a')
    rep.drawString(167,270,'unique and engaging fashion environment speaking to an opinion forming audience of trend-setters.')
    rep.drawString(160,240,'WaR aligns brands with influential fashion writers and internationally recognised publishers elevating')
...
def count_total_rates(metric,all_media):
    ...
    if metric=='Applause Rate':
        with suppress(Exception):
            rate= sum([media_likes_count(media, all_media) for media in all_media.keys()])/sum([media_posts_count(media,all_media) for media in all_media.keys()])
            return    str(("%.2f" %rate))
    if metric=='Conversation Rate':
        with suppress(Exception):
            rate= sum([media_comments_count(media, all_media) for media in all_media.keys()])/sum([media_posts_count(media,all_media) for media in all_media.keys()])
            return    str(("%.2f" %rate))
    if metric=='Amplification Rate':
        with suppress(Exception):
            rate= sum([media_shares_count(media, all_media) for media in all_media.keys()])/sum([media_posts_count(media,all_media) for media in all_media.keys()])
            return    str(("%.2f" %rate))
    if metric=='Interaction Rate':
        with suppress(Exception):
            rate= sum([media_interactions_count(media, all_media) for media in all_media.keys()])/sum([media_posts_count(media,all_media) for media in all_media.keys()])
            return    str(("%.2f" %rate))
    return toreturn
        ...
        for blogger in reshares[media].keys():
            for post in reshares[media][blogger].keys():
                for repost in reshares[media][blogger][post].keys():
                    impression+=reshares[media][blogger][post][repost]['impression'] #TODO is it correct
    return impression
    ...
    for metric in ['Posts','Interactions','Likes/Favourites','Comments','Shares/Retweets']:
        ...
        if metric=='Comments':
            if media1=='Twitter':
                ...
                rep.drawString(465,position,str("{:,}".format(count_media_results(media1,'Comments',all_media)))) 
                rep.drawString(700,position,str("{:,}".format(count_media_results(media1,'Comments',all_media)))) 
        if metric=='Shares/Retweets':
            if media1=='Instagram':
                rep.drawString(465,position,'---')
                rep.drawString(700,position,'---')
            else:
                rep.drawString(465,position,str("{:,}".format(count_media_results(media1,'Shares',all_media))))
                rep.drawString(700,position,str("{:,}".format(count_media_results(media1,'Shares',all_media))))
        if metric in ['Posts','Interactions','Likes/Favourites']:
            metric=metric.split('/')[0]
            rep.drawString(465,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
            rep.drawString(700,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
        counter+=30
    table_height=0
    ...
    for metric in ['Direct Reach/Uniques','Indirect Reach','Impressions']:
        ...
        if metric=='Indirect Reach':
            if media1=='Instagram':
                ...
                rep.drawString(465,position,str("{:,}".format(media_reach(media1,metric,followers_data,all_media,reshares))))
                rep.drawString(700,position,str("{:,}".format(total_reach(metric,followers_data,all_media,reshares))))
...
def two_social_media_reach_page(rep,all_media,followers_data,reshares):       
    ...
    for metric in ['Posts','Interactions','Likes/Favourites','Comments','Shares/Retweets']:
        ...
        if metric=='Comments':
            if media1=='Twitter':
                ...
                rep.drawString(515,position,str("{:,}".format(count_media_results(media2,'Comments',all_media))))
            if media2=='Twitter':
                rep.drawString(515,position,'---')
                rep.drawString(330,position,str("{:,}".format(count_media_results(media1,'Comments',all_media))))
        ...
        if metric=='Shares/Retweets':
            if media1=='Instagram':
                rep.drawString(330,position,'---')
                rep.drawString(515,position,str("{:,}".format(count_media_results(media2,'Shares',all_media))))
            else:
                rep.drawString(330,position,str("{:,}".format(count_media_results(media1,'Shares',all_media))))
                rep.drawString(515,position,str("{:,}".format(count_media_results(media2,'Shares',all_media))))
                
        if metric in ['Interactions','Likes/Favourites','Posts']:
            metric=metric.split('/')[0]
            rep.drawString(330,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
            rep.drawString(515,position,str("{:,}".format(count_media_results(media2,metric,all_media))))
        metric=metric.split('/')[0]
        rep.drawString(700,position,str("{:,}".format(count_media_results(media2,metric,all_media)+count_media_results(media1,metric,all_media))))
        counter+=30
    table_height=0
    rep.rect(650,280,175,30,fill=1)
    rep.setFillColor('white') #choose your font colour
    ...
    for metric in ['Interaction Rate.Interaction per post','Applause Rate.Likes per post','Conversation Rate.Comments per post','Amplification Rate. Shares per post']:       
        ...
        if metric=='Conversation Rate':
            if media1=='Twitter':
                rep.drawString(330,position,'---')
                rep.drawString(515,position,str(count_media_rates(media2,'Conversation Rate',all_media)))
            if media2=='Twitter':
                rep.drawString(515,position,'---')
                rep.drawString(330,position,str(count_media_rates(media1,'Conversation Rate',all_media)))
            if media1!='Twitter' and media2!='Twitter':
        ...
        if metric=='Amplification Rate':
            if media1=='Instagram':
                rep.drawString(330,position,'---')
                rep.drawString(515,position,str(count_media_rates(media2,'Amplification Rate',all_media)))
            else:
                rep.drawString(330,position,str(count_media_rates(media1,metric,all_media)))
                rep.drawString(515,position,str(count_media_rates(media2,metric,all_media)))
    ...
    for metric in ['Direct Reach','Indirect Reach','Impressions']:
        ...
        if metric=='Indirect Reach' and media1=='Instagram':
            ...
            rep.drawString(330,position,str("{:,}".format(media_reach(media1,metric,followers_data,all_media,reshares))))
        rep.drawString(515,position,str("{:,}".format(media_reach(media2,metric,followers_data,all_media,reshares))))
        rep.drawString(700,position,str("{:,}".format(total_reach(metric,followers_data,all_media,reshares))))
        counter+=30
    rep.showPage()
...
def social_media_reach_page(rep,all_media,followers_data,reshares):
    ...
    for metric in ['Posts','Interactions','Likes/Favourites','Comments','Shares/Retweets']:
        ...
        if metric=='Shares':
            ...
            rep.drawString(180,position,str("{:,}".format(count_media_results('Instagram',metric,all_media))))
        rep.setFont("Lato-Lig", 13 )
        if metric=='Comments':
            rep.setFont("Lato-Lig", 8) 
            rep.drawString(370,position,'---')
        else:
            rep.drawString(360,position,str("{:,}".format(count_media_results('Twitter',metric,all_media))))
        rep.setFont("Lato-Lig", 13 )
        rep.drawString(520,position,str("{:,}".format(count_media_results('Facebook',metric,all_media))))
        rep.drawString(700,position,str("{:,}".format(count_media_results('Facebook',metric,all_media)+count_media_results('Instagram',metric,all_media)+count_media_results('Twitter',metric,all_media))))
        counter+=30
    table_height=0
    rep.rect(650,280,175,30,fill=1)
    rep.setFillColor('white') #choose your font colour
    ...
    for metric in ['Direct Reach/Uniques','Indirect Reach','Impressions']:
        ...
        if metric=='Indirect Reach':
            ...
            rep.drawString(180,position,str("{:,}".format(media_reach('Instagram',metric,followers_data,all_media,reshares))))
        rep.setFont("Lato-Lig", 13)
        if metric=='Indirect Reach' and count_media_results('Facebook','Shares',all_media)!=0 and media_reach('Facebook','Indirect Reach',followers_data,all_media,reshares)==0:
            ...
            rep.drawString(520,position,str("{:,}".format(media_reach('Facebook',metric,followers_data,all_media,reshares))))
        rep.drawString(360,position,str("{:,}".format(media_reach('Twitter',metric,followers_data,all_media,reshares))))
        rep.drawString(700,position,str("{:,}".format(total_reach(metric,followers_data,all_media,reshares))))
        counter+=30
    rep.showPage()
...
def pdf(all_media,tags,bloggers,fromdate,todate,dirname,platforms,followers_data,size,brandname,campaign,reshares,blogposts,post_info):
    print ('Creating PDF....')
    rep = canvas.Canvas("%s/Desktop/%s/%s-%s.pdf" %(home,dirname,dirname,str(strftime("%Y_%m_%d_%H_%M_%S", gmtime()))), pagesize=A4)
    rep.setPageSize(landscape(A4))
