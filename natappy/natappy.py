...
def pagination_fb(blogger,fromdate,todate,logg_details,dirname):
    ...
    link = "https://graph.facebook.com/v2.5/%s?fields=likes,posts.until(%s).since(%s){link,message,shares,status_type,created_time,likes.summary(true),comments.summary(true)}&access_token=%s"%(blogger,until, since,logg_details['token'])         
    try:    
...
def pagination_ig(blogger,autho,fromdate,logg_details):
    ...
    try:
        while next_ and counter<300:  # limit 70*20
...
#    if blogger=='nothing':
#        return None
#    recent_media, next_ = autho.user_recent_media(user_id=username_to_id(blogger) )
#    counter=1
#    from_=datetime(fromdate[0],fromdate[1],fromdate[2])
#    day_before_from=from_ - timedelta(1)  
#    try:
#        while next_ and counter<300:            # limit 70*20
#            more_media, next_ = autho.user_recent_media(user_id=username_to_id(blogger),client_id=logg_details['client_id'], with_next_url=next_)
#            times_=[datetime(int(str(post.created_time)[:10].split('-')[0]),int(str(post.created_time)[:10].split('-')[1]),int(str(post.created_time)[:10].split('-')[2])) for post in more_media ]
#            if [time for time in times_ if time>=day_before_from]:        
#                recent_media.extend(more_media)
#                counter+=1 
#            else:
...
#    except Exception as e:
#        print (e)
#        raise Exception('Instagram pagination failed.')
#  
...
def pagination_tw_inside(blogger,twapi,replies=None,retweets=None):
    recent_media=[]  
    followers=0
    for tweet in tweepy.Cursor(twapi.user_timeline, twapi, screen_name=blogger, count=200, include_rts=True).items(limit=5000): 
...
def process_blogger_instagram(blogger,fromdate,logg_details,excludetags):
     ...
     autho = InstagramAPI(access_token=logg_details['token'],client_id=logg_details['client_id'], client_secret=logg_details['client_secret'])           
...
def process_blogger_twitter(blogger,fromdate,logg_details,replies,retweets,excludetags):
    ...
    print ('Twitter limits left:', autho.rate_limit_status()['resources']['statuses']['/statuses/user_timeline']['remaining'])
    return recent_media,followers,autho
...
def process_blogger(blogger,platform,tags,repformat,fromdate,todate,followers_data,replies,retweets,excludetags,dirname):
    ...
    if platform=='Facebook': 
        blogger_posts_info=process_blogger_facebook(blogger,fromdate,todate,logg_details=logg_details,excludetags=excludetags,dirname=dirname)
    if platform=='Instagram':
        blogger_posts_info=process_blogger_instagram(blogger,fromdate,logg_details=logg_details,excludetags=excludetags)
    if platform=='Twitter':
        blogger_posts_info=process_blogger_twitter(blogger,fromdate,logg_details=logg_details,replies=replies,retweets=retweets,excludetags=excludetags)
    ...
    if recent_media:
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            result = list(executor.map(partial(process_post, tags=tags,autho=autho, platform=platform, repformat=repformat, blogger=blogger, followers=followers,fromdate=fromdate,todate=todate,replies=replies,retweets=retweets,excludetags=excludetags,dirname=dirname), recent_media))
...
def get_post_with_tags(bloggers,platform,tags,repformat,fromdate,todate,followers_data,replies,retweets,excludetags,dirname):   
    ...
    results = executor_map(partial(process_blogger, platform=platform, tags=tags,repformat=repformat,fromdate=fromdate,todate=todate,followers_data=followers_data,replies=replies,retweets=retweets,excludetags=excludetags,dirname=dirname ), bloggers)
...
def process_post_twitter(post,blogger,tags,followers,from_,to_,replies,retweets,excludetags):
    time_=datetime(int(str(post['created_at'])[:10].split('-')[0]),int(str(post['created_at'])[:10].split('-')[1]),int(str(post['created_at'])[:10].split('-')[2]))        
    ...
    try:
        ...
        if os.path.getsize(screenshot)>200:
            ...
            if note=='shared_story':
                if img.size[1]<=734:
                    img.crop((190,136,img.size[0]-300,img.size[1]-170)).save('%s' %screenshot)
                    time.sleep(2)
                if img.size[1]>734:
                    img.crop((190,132,img.size[0]-300,img.size[1]-100)).save('%s' %screenshot)
                    time.sleep(2)
            if note.startswith('mobile_status_update'):  
                img.crop((190,140,img.size[0]-303,img.size[1]-465+16*int(note.split(',')[1]))).save('%s' %screenshot)
            if note=='added_video':               
                img.crop((25,543,img.size[0]-143,img.size[1]-33)).save('%s' %screenshot)
                time.sleep(2)
            if note.startswith('shared_utube'):
                img.crop((190,138,img.size[0]-300,img.size[1]-293+16*int(note.split(',')[1]))).save('%s' %screenshot)                                
                time.sleep(2)
        ...
        if platform=='Twitter':             
            ...
            try:
              #  element=browser.find_element_by_css_selector('div.expansion-container.js-expansion-container')
                element=browser.find_element_by_css_selector('div.permalink-inner.permalink-tweet-container')
                browser.save_screenshot('%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,platform,name)) 
                size=element.size
                location=element.location
                note=[int(location['x']),int(location['y']),int(size['width']),int(size['height'])]
                browser.quit()
              #  note=[50,50,1366,1500]
                cut_screenshot('%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,platform,name),url=url,note=note,platform=platform)
...
def collect_screenshots(dirname):
    ...
    try:
        print('Screenshots collected. Elapsed:', time.time()-tic, 'Average:',int(time.time()-tic)/len(links),'per post')
    except:
        pass
    ...
    for media in all_media.keys():
        for blogger in all_media[media].keys():  
            ...
            for post in all_media[media][blogger].keys():  
                post_details=[media,linkname_to_screenname(blogger,media),followers_data[linkname_to_screenname(blogger,media)][media],str(post),all_media[media][blogger][post]['link'],all_media[media][blogger][post]['created at'][:10],all_media[media][blogger][post]['created at'][10:],all_media[media][blogger][post]['text'],all_media[media][blogger][post]['likes'],all_media[media][blogger][post]['shares'],all_media[media][blogger][post]['comments'],all_media[media][blogger][post]['likes']+all_media[media][blogger][post]['shares']+all_media[media][blogger][post]['comments'],float(all_media[media][blogger][post]['engagement']),counter]
...
def excel(all_media,dirname,tags,followers_data):  
    ...
    fieldsnames= ('MEDIA','BLOGGER','FOLLOWERS','POSTS_ID','POSTS_LINK','CREATED_DATE','CREATED_TIME','POSTS_TEXT','POSTS_LIKES','POSTS_SHARES','POSTS_COMMENTS','POST_INTERACTIONS','POSTS_ENGAGEMENT')
#    columns=['A','B','C','D','E','F','G','H','I','J','K','L','M']
    ...
    for post in posts:
        col=0              
        for name in post[:13]:
            new_image_name=''.join([name_part for name_part in post[1]])+'_%s_(%s)' %(tags[0],post[13])
            with suppress(Exception):
                os.rename('%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,post[0],post[3]),'%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,post[0],new_image_name) )
...
def addtional_Excel(dirname):
    ...
    TotalMedia=pandas.pivot_table(df,index=["MEDIA"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    DayByDayTotal=pandas.pivot_table(df,index=["CREATED_DATE"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    DayByDay=pandas.pivot_table(df,index=["MEDIA","CREATED_DATE"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    BloggersTotals=pandas.pivot_table(df,index=["BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    Bloggers=pandas.pivot_table(df,index=["MEDIA","BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    HourByHourTotal=pandas.pivot_table(df,index=["CREATED_TIME"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    HourByHour=pandas.pivot_table(df,index=["MEDIA","CREATED_TIME"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    MediaByBlogger=pandas.pivot_table(df,index=["MEDIA","BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    
    Facebook=pandas.pivot_table(fb,index=["BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    Instagram=pandas.pivot_table(ig,index=["BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
    Twitter=pandas.pivot_table(tw,index=["BLOGGER"],values=["POSTS_ID","FOLLOWERS","POST_INTERACTIONS","POSTS_LIKES","POSTS_COMMENTS","POSTS_SHARES","POSTS_ENGAGEMENT"],aggfunc={"POSTS_ID":len,"FOLLOWERS":np.sum,"POST_INTERACTIONS":np.sum,"POSTS_LIKES":np.sum,"POSTS_COMMENTS":np.sum,"POSTS_SHARES":np.sum,"POSTS_ENGAGEMENT":np.average})
...
def count_media_impression(all_media,media,followers_data):
    ...
    for blogger in all_media[media]:
        media_impression+=followers_data[linkname_to_screenname(blogger,media)][media]*len(all_media[media][blogger])        
...
def cut_blog_screenshots(dirname,screenshot,blogger,counter):
    ...
    pieces=[]
    for i in range(n):    
        img.crop((0,0+i*new_height,img.size[0],0+i*new_height+new_height)).save('%s/Desktop/%s/Blogs_screenshots/%s_(%s)_(%s).png' %(home,dirname,blogger,counter,i))
        pieces.append('%s/Desktop/%s/Blogs_screenshots/%s_(%s)_(%s).png' %(home,dirname,blogger,counter,i))
    return pieces
...
def grab_post_screenshot(dirname,link,blogger,counter):
    ...
    while(match==False):
        ...
        if lastCount==lenOfPage:
            match=True
    time.sleep(3)   
    browser.save_screenshot('%s/Desktop/%s/Blogs_screenshots/%s_(%s).png' %(home,dirname,blogger,counter)) 
    browser.quit()  
    n=cut_blog_screenshots(dirname,'%s/Desktop/%s/Blogs_screenshots/%s_(%s).png' %(home,dirname,blogger,counter),blogger,counter) 
    return n
...
def collect_blog_screenshots(dirname,blogposts):
    ...
    for blogger in blogposts:
        ...
        for link in blogposts[blogger]:
            ...
            post_info[blogger][link]['pieces']=grab_post_screenshot(dirname,link,blogger,counter)
            counter+=1
    return post_info
...
def create_report(screenbloggers,platforms,tags,repformat,fromdate,todate,replies,retweets,excludetags,brandname,campaign,blogposts): 
  #  print ('repformat:', repformat)
    ...
    dirname= '%s-%s-%s' %(platshorts,'_'.join([tag for tag in tags]),str(strftime("%Y_%m_%d_%H_%M", gmtime()))) 
    pickle.dump(dirname, open('{home}/Desktop/natappy/pickle/dirname.pickle'.format(home=home), 'wb'))
    with suppress(Exception):
    ...
    for platform in [plat for plat in platforms if plat not in['Blogs']]: 
        all_media[platform]=get_post_with_tags(screenbloggers,platform,tags,repformat,fromdate,todate,followers_data,replies,retweets,excludetags,dirname) 
    pickle.dump(all_media, open('{home}/Desktop/natappy/pickle/all_media.pickle'.format(home=home), 'wb'))
    pickle.dump(followers_data, open('{home}/Desktop/natappy/pickle/followers.pickle'.format(home=home), 'wb'))
    pickle.dump(links, open('{home}/Desktop/natappy/pickle/links.pickle'.format(home=home), 'wb')) 
    dirname ='report'
    dirname= pickle.load(open('{home}/Desktop/natappy/pickle/dirname.pickle'.format(home=home), 'rb')) 
    all_media= pickle.load(open('{home}/Desktop/natappy/pickle/all_media.pickle'.format(home=home), 'rb'))
    followers_data=pickle.load(open('{home}/Desktop/natappy/pickle/followers.pickle'.format(home=home), 'rb'))
    ...
    pickle.dump(reshares, open('{home}/Desktop/natappy/pickle/reshares.pickle'.format(home=home), 'wb'))
    reshares=pickle.load(open('{home}/Desktop/natappy/pickle/reshares.pickle'.format(home=home), 'rb'))
    post_info=pickle.load(open('{home}/Desktop/natappy/pickle/postsinfo.pickle'.format(home=home), 'rb'))
    if reshares:
    ...
    if repformat=='Full':
        if 'Blogs' in platforms:
            ...
            pickle.dump(post_info, open('{home}/Desktop/natappy/pickle/postsinfo.pickle'.format(home=home), 'wb'))
        collect_screenshots(dirname)
    newpdf.pdf(all_media,tags,screenbloggers,fromdate,todate,dirname=dirname,platforms=platforms,followers_data=followers_data,size='full',brandname=brandname,campaign=campaign,reshares=reshares,blogposts=blogposts,post_info=post_info)
    ...
    if repformat=='Quick':
        if 'Blogs' in platforms:
            ...
            post_info=collect_blog_screenshots(dirname,blogposts)
            pickle.dump(post_info, open('{home}/Desktop/natappy/pickle/postsinfo.pickle'.format(home=home), 'wb'))         
        newpdf.pdf(all_media,tags,screenbloggers,fromdate,todate,dirname,platforms,followers_data,repformat,brandname=brandname, campaign=campaign,reshares=reshares,blogposts=blogposts,post_info=post_info)
        excel(all_media,dirname,tags,followers_data)    
        addtional_Excel(dirname)
    time.sleep(2)
...
if __name__ == '__main__':
    info = {'blogpost': {'Becca Rose': [], 'Chanel Boateng': [], 'Fashion Mumblr': ['http://www.fashionmumblr.com/2016/10/natural-way-finding-confidence-skin'], 'Hannah Maggs': []}, 'retweets': False, 'excludetags': ['Exclude tag'], 'comptags': [], 'competition': [], 'brandname': '', 'replies': False, 'format': 'Full', 'blogposts': [], 'bloggers': ['Becca Rose', 'Chanel Boateng', 'Fashion Mumblr', 'Hannah Maggs'], 'todate': [2016, 11, 17], 'fromdate': [2016, 10, 19], 'tags': ['garnier'], 'campaign': '', 'platforms': ['Facebook', 'Instagram', 'Twitter', 'Blogs']}
  ...
  #  info= {'platforms': ['Facebook', 'Instagram', 'Twitter'], 'competition': [], 'excludetags': ['Exclude tag'], 'retweets': False, 'todate': [2016, 2, 17], 'bloggers': ['Hannah Louise Fashion'], 'blogpost': {}, 'comptags': [], 'tags': ['a'], 'format': 'Full', 'blogposts': [], 'replies': False, 'campaign': '', 'fromdate': [2016, 2, 14], 'brandname': ''}
   # info= {'tags': ['a'], 'format': 'Quick', 'campaign': '17 february', 'fromdate': [2016, 2, 10], 'blogposts': [], 'bloggers': ['Alice Point', 'Amelia Liana', 'Anneli Bush'], 'comptags': [], 'replies': False, 'blogpost': {}, 'retweets': False, 'excludetags': ['Exclude tag'], 'brandname': 'some report', 'platforms': ['Facebook', 'Instagram', 'Twitter'], 'competition': [], 'todate': [2016, 2, 17]}
    create_report(info['bloggers'],info['platforms'],info['tags'],info['format'],info['fromdate'],info['todate'],info['replies'],info['retweets'],info['excludetags'],info['brandname'],info['campaign'],info['blogpost'])
