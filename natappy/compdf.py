# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 15:38:08 2015
...
def find_screenshots(dirname,all_media):
    ...
    for media in all_media.keys():
        for post in all_media[media].keys():
                post_path= '%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,media, all_media[media][post]['authortw']+'_'+all_media[media][post]['post_id'])
...
def find_other_screenshots(dirname,bloggers,all_media): 
    ...
    for media in all_media.keys():
        for post in all_media[media].keys():
            post_path= '%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,media,all_media[media][post]['authortw']+'_'+ str(all_media[media][post]['post_id']))
            try:                    
                img=Image.open(post_path) 
            except:
                post_path= '%s/Desktop/%s/%s_screenshots/%s.png' %(home,dirname,media,str(all_media[media][post]['post_id']))
                img=Image.open(post_path) 
            ratio=img.size[0]/img.size[1]
            if ratio<=1:
                position='portrait'
...
def welcome_pages(rep,brandname,campaign):
    rep.drawImage('%s/Desktop/natappy/images/first.png' %home,250,280,400,300)  #260,320,300,100
...
def two_social_media_reach_page(rep,all_media,followers_data):
    ...
    for metric in ['Posts','Interaction','Likes/Favourites','Comments','Shares/Retweets']:
        ...
        if metric=='Shares':
            ...
            rep.drawString(565,position,str("{:,}".format(count_media_results(media2,'Shares',all_media))))
        if metric=='Comments':
        ...
        if metric in ['Posts','Interaction','Likes']:
            ...
            rep.drawString(565,position,str("{:,}".format(count_media_results(media2,metric,all_media))))
        rep.drawString(700,position,str("{:,}".format(count_media_results(media1,metric,all_media)+count_media_results(media2,metric,all_media))))
        counter+=30
    table_height=0
    for i in range(5):
    ...
    for metric in ['Interactions Rate.Interaction per post','Applause Rate.Likes per post','Conversation Rate.Comments per post','Amplification Rate.Shares per post']:
        ...
        if metric=='Amplification Rate':
            rep.drawString(330,position,'---')
            rep.drawString(565,position,str(count_media_rates(media2,'Amplification Rate',all_media)))
    ...
    for metric in ['Impression']:
        ...
        rep.drawString(30,position,metric)
        rep.setFont("Lato-Lig", 13)
        rep.drawString(330,position,str("{:,}".format(media_reach(media1,metric,followers_data,all_media))))
        rep.drawString(565,position,str("{:,}".format(media_reach(media2,metric,followers_data,all_media))))
        rep.drawString(700,position,str("{:,}".format(total_reach(metric,followers_data,all_media))))
        counter+=30
    rep.showPage()
...
def one_social_media_reach_page(rep,all_media,followers_data):
    ...
    for metric in ['Posts','Interaction','Likes/Favourites','Comments','Shares/Retweets']:
        ...
        if metric=='Shares':
            if media1=='Instagram':
                ...
                rep.drawString(720,position,str("{:,}".format(count_media_results(media1,'Shares',all_media))))
        if metric=='Comments':
            if media1=='Twitter':
                rep.drawString(465,position,'---')
                rep.drawString(720,position,'---')
                ...
                rep.drawString(720,position,str("{:,}".format(count_media_results(media1,'Comments',all_media))))
        if metric in ['Posts','Interaction','Likes']:
            metric=metric.split('/')[0]
            rep.drawString(465,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
            rep.drawString(720,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
        counter+=30
    table_height=0
    for i in range(5):
        rep.line(20,230-30*i,825,230-30*i)
    ...
    for metric in ['Interactions Rate.Interaction per post','Applause Rate.Likes per post','Conversation Rate.Comments per post','Amplification Rate.Shares per post']:
        ...
        if metric=='Conversation Rate':
            if media1=='Twitter':
                ...
                rep.drawString(465,position,str(count_media_rates(media1,'Conversation Rate',all_media)))
                rep.drawString(720,position,str(count_total_rates('Conversation Rate',all_media)))
        if metric=='Amplification Rate':
            if media1=='Instagram':
                rep.drawString(465,position,'---')
                rep.drawString(720,position,'---')
            else:
                rep.drawString(465,position,str(count_media_rates(media1,'Amplification Rate',all_media)))
                rep.drawString(720,position,str(count_total_rates('Amplification Rate',all_media)))
        if metric in ['Interactions Rate','Applause Rate']:
    ...
    for metric in ['Impression']:
        ...
        rep.drawString(30,position,metric)
        rep.setFont("Lato-Lig", 13)
        rep.drawString(465,position,str("{:,}".format(media_reach(media1,metric,followers_data,all_media))))
        rep.drawString(720,position,str("{:,}".format(total_reach(metric,followers_data,all_media))))
        counter+=30
    rep.showPage()
...
def social_media_reach_page(rep,all_media,followers_data):
    ...
    for metric in ['Posts','Interaction','Likes','Comments','Shares']:
        ...
        if metric in ['Likes','Comments','Shares']:
            ...
            rep.drawString(30,position,metric)
        rep.setFont("Lato-Reg", 13)
        rep.drawString(240,position,str("{:,}".format(count_media_results(media1,metric,all_media))))
        rep.drawString(420,position,str("{:,}".format(count_media_results(media2,metric,all_media))))
        rep.drawString(580,position,str("{:,}".format(count_media_results(media3,metric,all_media))))
        rep.drawString(710,position,str("{:,}".format(count_media_results('Facebook',metric,all_media)+count_media_results('Instagram',metric,all_media)+count_media_results('Twitter',metric,all_media))))
        counter+=30
    table_height=0
    for i in range(5):
        rep.line(20,230-30*i,825,230-30*i)
    ...
    for metric in ['Impression']:
        ...
        rep.drawString(30,position,metric)
        rep.setFont("Lato-Lig", 13)
        rep.drawString(240,position,str("{:,}".format(media_reach(media1,metric,followers_data,all_media))))
        rep.drawString(420,position,str("{:,}".format(media_reach(media2,metric,followers_data,all_media))))
        rep.drawString(580,position,str("{:,}".format(media_reach(media3,metric,followers_data,all_media))))
        rep.drawString(710,position,str("{:,}".format(total_reach(metric,followers_data,all_media))))
        counter+=30
    rep.showPage()
...
def excel(dirname,our_posts,followers_data):  
    ...
    fieldsnames= ('MEDIA','IMPRESSION','AUTHOR','AUTHOR_FOLLOWERS','POST_ID','TEXT','CREATED_AT','LINK','LIKES','COMMENTS','SHARES','ENGAGEMENT')
